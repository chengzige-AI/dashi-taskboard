...
type IntervalSeconds = 5 | 10 | 30 | 60 | 300 | 600 | 900 | 1800 | 3600;
  ...
  return (
    <>
      <button
        ...
        aria-label={automation ? stateLabel : "未配置"}
        aria-busy={pending}
        aria-haspopup="dialog"
        aria-expanded={open}
        title={automation ? stateLabel : "未配置"}
        onClick={() => {
          if (!open) {
      ...
      >
        ...
        <span>{automation ? (automation.enabledByUser ? "自动认领" : "已暂停") : "未配置"}</span>
      </button>
      {menu}
    </>
  );
