...
async function main() {
  ...
  console.log(`codex-kanban listening on http://127.0.0.1:${address.port}`);
  if (host === "0.0.0.0") {
    ...
    for (const lanAddress of [...new Set(addresses)]) {
      console.log(`codex-kanban available on LAN at http://${lanAddress}:${address.port}`);
    }
  }
